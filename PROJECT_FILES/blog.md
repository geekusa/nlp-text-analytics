...
I set out to bring natural language processing to Splunk... did I accomplish this? Yes I would say so, though lots of bumps and bruises along the way and definitely still room for improvement. Text analytics is hard. This probably comes down to the fact that once the data is tokenized, there are just so many features per document to begin with. Not to mention human language has so many nuisances. Though when you come down to it, text analytics and unsupervised learning in general, is not an exact science
